lambda_packages: lambda_packages/weekly_reminder.zip \
		 		 lambda_packages/lambda_query_api.zip \
...
lambda_packages/lambda_query_api.zip: app/lambda_query_api.py \
								app/support/record_formatting.py \
								app/support/notifications.py \
								app/support/common/ \
								app/support/data_access_layer/get_records.py \
								app/support/common/aws_resources.py \
								app/support/data_access_layer/records/pet_table_models.py
	echo "Building lambda_query_api Lambda zip package"
	echo "Removing old directory and zip file"
	if [ -d lambda_packages/lambda_query_api ]; then rm -rf lambda_packages/lambda_query_api; fi
	if [ -e lambda_packages/lambda_query_api.zip ]; then rm lambda_packages/lambda_query_api.zip; fi
	echo "Creating directory structure"
	mkdir -p lambda_packages/lambda_query_api/support/data_access_layer/records
	mkdir -p lambda_packages/lambda_query_api/support/common

	echo "Copying files"

	cp app/__init__.py                  lambda_packages/lambda_query_api/__init__.py
	cp app/lambda_query_api.py          lambda_packages/lambda_query_api/lambda_query_api.py

	cp app/support/__init__.py 	         lambda_packages/lambda_query_api/support/__init__.py
	cp app/support/record_formatting.py  lambda_packages/lambda_query_api/support/record_formatting.py
	cp app/support/notifications.py      lambda_packages/lambda_query_api/support/notifications.py
	cp app/support/find_reminders.py     lambda_packages/lambda_query_api/support/find_reminders.py

	cp -r app/support/common    lambda_packages/lambda_query_api/support/

	cp app/support/data_access_layer/__init__.py       lambda_packages/lambda_query_api/support/data_access_layer/__init__.py
	cp app/support/data_access_layer/get_records.py    lambda_packages/lambda_query_api/support/data_access_layer/get_records.py

	cp app/support/data_access_layer/records/__init__.py            lambda_packages/lambda_query_api/support/data_access_layer/records/__init__.py
	cp app/support/data_access_layer/records/pet_table_models.py    lambda_packages/lambda_query_api/support/data_access_layer/records/pet_table_models.py

	echo "Tidying up __pycache__ directories"
	find ./lambda_packages/ -type d -name __pycache__ | xargs rm -rf

	echo "Zipping up package"
	cd lambda_packages/lambda_query_api; zip -r lambda_query_api.zip .; cd ..; mv lambda_query_api/lambda_query_api.zip ./
